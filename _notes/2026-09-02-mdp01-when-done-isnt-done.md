...
I wrote about the terminal invariant problem above — hardcoded status enumeration across four modules, fragile to new states. Then I audited CaseStatus and found exactly that.

CaseStatus was the only lifecycle enum in the platform without `isTerminal()` or `isActive()`. Seven values, no lifecycle methods, not registered in LIFECYCLE.md. Every other state machine — WorkItemStatus, PlanItemStatus, CommitmentState, GroupStatus — had been through the alignment. CaseStatus hadn't.

So I added the methods and went looking for hardcoded terminal checks. Fifteen sites in the engine. Two explicit `Set.of(COMPLETED, FAULTED, CANCELLED)` constants. Seven handler classes with their own `private static boolean isTerminal()` that shadows what should be on the enum. All of them silently incorrect the moment COMPENSATED is added.

This is the validation for the separate-entity decision. Adding COMPENSATING and COMPENSATED to WorkItemStatus — the BPMN approach — would have created exactly this class of bug across a wider surface. The twelve-value WorkItemStatus has more consumers than the seven-value CaseStatus. The audit would have been larger, the breakage more scattered.

The ledger work was the opposite experience. Adding CompensationSupplement followed the existing supplement pattern — API abstract class, JPA entity, orm.xml mappings, serializer type key. The two-layer model keeps JPA annotations out of the public API jar, so consumers depend on the supplement type without pulling in Hibernate. No surprises, no fragility. Consistency paying its dividends.
